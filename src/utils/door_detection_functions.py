...
from utils.sound_functions import compute_directional_sound, SoundPlayer
...
def box_sound_detection(frame, model, player):
    ...
    for box, score in zip(smoothed_boxes, smoothed_scores):
        ...
        if door_n == 0:
            # print(f"Door {door_n}: Center at {box_center}, Score: {score:.2f}, Image center: {image_center}")
            stereo, rate = compute_directional_sound(image_center, 0, box_center)
            player.play(stereo, rate)
